...
Film texture update: switch between Off, Fine, Classic and Heavy on the same sample, including Silver and Noir. Use 3× detail to compare texture on matching Film/Original crops. Film/Original must show the right image after rapid stock, scene and grain changes. Load Heavy grain, relaunch, and confirm the Darkroom roll card retains Heavy. Existing installations should still open old rolls and show Classic as their default. After development, inspect grain at full size and check that bright sources have gentle glow on Daylight, Amber, Dusk and Faded without washing out dark scenes.

Export update: save a whole developed roll and verify saved/total progress. Press Stop partway through; at most the photo currently being saved should finish. The reported saved count must match Photos, and the full roll must remain in Darkroom. Check denial of Photos permission and repeat exports, which still make additional copies.
